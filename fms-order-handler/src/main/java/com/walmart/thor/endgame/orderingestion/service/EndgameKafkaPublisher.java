...
public class EndgameKafkaPublisher {
  ...
  public EndgameKafkaPublisher(@Qualifier("endgameKafkaProducer") KafkaTemplate<String,
      String> kafkaTemplate, @Value("${spring.kafka.producer.customer-cancellation-topic}") String customerOrderTopic){
      ...
      future.addCallback(
          new ListenableFutureCallback<>() {
            ...
            public void onFailure(Throwable e) {
              log.error(
                  "failed to publish fmsorder cancellation to kafka: message: {}, error: {}", message, e);
            }

            @Override
            public void onSuccess(SendResult<String, String> result) {
              String key = result.getProducerRecord().key();
              log.info("fmsorder cancellation published to kafka, key: {}", key);
            }
          });
    }
  }
